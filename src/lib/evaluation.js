...
function evaluateMaterial(board) {
  ...
  board.forEach((row, rowIndex) => {
    row.forEach((piece, colIndex) => {
      ...
      const advancement =
        piece.type === 'p'
          ? (piece.color === 'w' ? rowIndex : 7 - rowIndex) * -0.02
          : 0
...
export function evaluatePosition(chessLike) {
  ...
  if (chess.isCheckmate()) return chess.turn() === 'w' ? -99 : 99
  ...
  // ── Safely compute opponent mobility ──────────────────────────────────────
  // Swapping the turn in a FEN can produce an illegal position (e.g. king left
  // in check), so we guard with try/catch instead of crashing rankLegalMoves.
  let blackMobility = 0
  try {
    const fenParts = chess.fen().split(' ')
    fenParts[1] = turn === 'w' ? 'b' : 'w'
    const enemy = new Chess(fenParts.join(' '))
    blackMobility = enemy.moves().length
  } catch {
    blackMobility = whiteMobility // neutral fallback — no crash
  }

  const mobility = (whiteMobility - blackMobility) * 0.025
  const kingPressure = chess.inCheck() ? (turn === 'w' ? -0.35 : 0.35) : 0
  ...
  return moves
    .map((move) => {
      try {
        ...
        return { move, whiteEval, moverEval }
      } catch {
        return null
      }
    })
...
export function classifyMove(bestMoverEval, chosenMoverEval) {
  ...
  return (
    CLASSIFICATIONS.find((item) => loss <= item.maxLoss) ??
    CLASSIFICATIONS[CLASSIFICATIONS.length - 1]
  )
}

export function formatEval(score) {
  if (Math.abs(score) >= 90) return score > 0 ? 'M+' : 'M-'
